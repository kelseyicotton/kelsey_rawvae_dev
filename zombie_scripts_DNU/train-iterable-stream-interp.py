...
for epoch in range(epochs):
    ...
    if epoch % checkpoint_interval == 0 and epoch != 0:
        ...
        # INTEGRATE INTERPOLATIONS FUNCTIONALITY HERE
        if generate_test:
            # GET RANDOM AUDIO FOR INTERPOLATION
            audio_file1, audio_file2 = get_random_audio(dataset_test_audio)
            ...
            logging.info(f"Matched lengths: {len(audio1_matched)} samples")
            ...
            logging.info(f"Created tensors of shape: {audio_tensor1.shape}")
            ...
            # INTERPOLATE
            interpolation_output_dir = workdir / "interpolations"
            create_interpolations(model, z1, z2, sampling_rate, interpolation_output_dir)
