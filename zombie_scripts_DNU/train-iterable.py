...
from rawvae.dataset import AudioDataset, AudioIterableDataset, ToTensor
...
logging.info('Total number of audio frames: {}'.format(total_frames))
...
# Check this works, and hot swapping

training_dataset = AudioIterableDataset(training_array, segment_length = segment_length, sampling_rate = sampling_rate, hop_size = hop_length, transform=ToTensor())
...
logging.info(f'Total segments available: {training_dataset.total_segments}') # 🆕

logging.info(f'Segment length: {training_dataset.segment_length}') # 🆕

logging.info(f'Hop size: {training_dataset.hop_size}') # 🆕
